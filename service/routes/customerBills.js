				...
				if (err) {
					...
					if (count > 0) {
						...
						const debtOrders = orders.filter(
							(order) => order.totalAmount !== order.paymentAmount
						);
						...
						debtOrdersClone.map((order) => {
							...
							if (order["customerId"]) {
								if (customerObj[order["customerId"]]) {
									customerObj[order["customerId"]].push(
										order
									);
								} else {
						...
						//欠债的客户列表
						const debtCustomers = Object.keys(customerObj).map(
							(key) => {
								let debtCustomer = {};
						...
						const bills = Object.keys(customerObj).map((key) => {
							...
							billObj["totalAmount"] = customerObj[key].reduce(
								(total, order) => (total += order.totalAmount),
								0
							);
							billObj["paymentAmount"] = customerObj[key].reduce(
								(total, order) =>
...
router.route("/doClearOrder").post(function (req, res, next) {
	let currentUser = req.session.userInfo;
