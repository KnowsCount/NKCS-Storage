...
function getResource(queryCondition, callback) {
	Order.find(queryCondition, (err, orders) => {
		...
		orders
			...
			.reduce(
				(products, itemProducts) => products.concat(itemProducts),
				[]
			)
			.map((product) => {
		...
		let orderComputeProducts = Object.keys(productGroup).map((key) => {
			...
			/*销售均价*/
			computedObj["averagePrice"] =
				compute(productGroupItem, "price") / productGroupItem.length;
			return computedObj;
		});
		...
		Storage.find(queryCondition, (err, storages) => {
			...
			let storageComputeProducts = Object.keys(productGroup).map(
				(key) => {
					const computedObj = {};
					const productGroupItem = productGroup[key];
					computedObj["_id"] = key;
					computedObj["inAmount"] = compute(
						productGroupItem,
						"quantity"
					);
					computedObj["purchasePrice"] = compute(
						productGroupItem,
						"amount"
					);
					computedObj["storageAveragePrice"] =
						compute(productGroupItem, "price") /
						productGroupItem.length;
					computedObj["productUnit"] =
						productGroupItem[0]["productUnit"];
					return computedObj;
				}
			);
			...
			Product.find({}, (err, products) => {
				 ...
				 * */
				/*将进货单和出货单中的商品再次进行一次 分组汇总*/
				const concatProducts = orderComputeProducts.concat(
					storageComputeProducts
				);
				...
				const productMapResult = Object.keys(concatProductGroup)
					.map((key) => {
						return concatProductGroup[key].reduce(
							(assignObj, product) =>
								Object.assign({}, assignObj, product),
							{}
						);
					})
					.map((product) => {
						...
						product["outAmount"] = product["outAmount"]
							? product["outAmount"]
							: 0;
						product["amount"] =
							product["inAmount"] - product["outAmount"];

						product["averagePrice"] = product["averagePrice"]
							? product["averagePrice"]
							: product["storageAveragePrice"];
						product["stockFunds"] =
							product["amount"] * product["averagePrice"];
						...
						Order.find(
							...
							function (error, orders) {
								if (error) {
									...
									/*生成订单*/
									let products = productMapResult.map(
										(product, index) => {
											return {
												remarks: "结算商品",
												amount: 0,
									...
									const order = new Order({
										sequence: sequence,
										orderNumber: utils.getOrderNumber(
											sequence
										),
										customerId: "",
										totalAmount: 0,
											...
											ProductStocks.insertMany(
												...
												(err) => {
													if (err) {
														...
														/*第二步，生成结算单*/
														const settlementAmount = productMapResult.reduce(
															(total, product) =>
																(total +=
																	product[
																		"profitPrice"
																	...
																	return {
																		...
																		key: index
																				...
																				const storage = new Storage(
																					{
																						sequence: sequence,
																						noteNumber: utils.getNoteNumber(
																							sequence
																						),
																						supplierId:
																							"",
