		...
		Storage.find(queryCondition)
			...
			.exec(function (err, storages) {
				if (err) {
					...
					if (count > 0) {
						//负债入库单
						const debtStorages = storages.filter(
							(storage) =>
								storage.totalAmount !== storage.paymentAmount
						);
						...
						debtStoragesClone.map((storage) => {
							...
							if (storage["supplierId"]) {
								if (supplierObj[storage["supplierId"]]) {
									supplierObj[storage["supplierId"]].push(
										storage
									);
								} else {
									supplierObj[storage["supplierId"]] = [
										storage,
									];
								}
							}
						...
						//负债的供应商列表
						const debtSuppliers = Object.keys(supplierObj).map(
							(key) => {
								let debtSupplier = {};
						...
						const bills = Object.keys(supplierObj).map((key) => {
							...
							billObj["totalAmount"] = supplierObj[key].reduce(
								(total, storage) =>
									(total += storage.totalAmount),
								0
							);
							billObj["paymentAmount"] = supplierObj[key].reduce(
								(total, storage) =>
									(total += storage.paymentAmount),
								0
							);
							billObj["debtAmount"] =
								billObj["totalAmount"] -
								billObj["paymentAmount"];
							return billObj;
						});
						...
						Supplier.find({}, function (err, suppliers) {
							...
							debtStorages.map((storage) => {
								...
								storage["supplierName"] =
									supplierMap[storage["supplierId"]];
							});
							debtSuppliers.map(
								(supplier) =>
									(supplier["supplierName"] =
...
router.route("/doClearStorage").post(function (req, res, next) {
	let currentUser = req.session.userInfo;
...
router.route("/doClearBill").post(function (req, res, next) {
	let currentUser = req.session.userInfo;
