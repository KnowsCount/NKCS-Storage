...
export default {
	...
	effects: {
		*query({ payload }, { call, put, select }) {
			...
			let { page, timeRange, supplierId, noteNumber } = yield select(
				(state) => state.storage
			);
			supplierId = supplierId == "00000" ? "" : supplierId;
			const { data } = yield call(
				query,
				parse({ page, timeRange, supplierId, noteNumber })
			);
			if (data) {
				yield put({
					type: "querySuccess",
		...
		*modify({ payload }, { select, call, put }) {
			...
			const id = yield select(
				({ storage }) => storage.currentItem["_id"]
			);
	...
	reducers: {
		...
		delSuccess(state, action) {
			const newList = state.list.filter(
				(storage) => storage._id != action.payload
			);
			return { ...state, list: newList, loading: false };
		},
		...
		resetStorage(state, action) {
			...
			let storageData = Object.assign({}, defaultStorage, {
				products: [{ ...defaultProduct }],
			});
			return {
				...state,
